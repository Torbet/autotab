...
def visualize_jams_note(jam, save_path=None):
  ...
  for string_tran in annos:
    ...
    for note in string_tran:
      ...
      midi_note = note[2]
      dur = note[1]
      plt.plot([start_time, start_time + dur], [midi_note, midi_note], style_dict[s], label=string_dict[s])
    s += 1
  plt.xlabel('Time (sec)')
...
def tablaturize_jams(jam, save_path=None):
  ...
  for string_tran in annos:
    ...
    for note in string_tran:
      ...
      plt.scatter(start_time, s + 1, marker='${}$'.format(fret), color=style_dict[s])
    s += 1
...
def add_annotations_to_barline(ax, annotations, beat_annotations, ygrow_ratio=1, label_xoffset=0, label_yoffset=0):
  """
...
def tokenize_jams(jam):
  str_midi_dict = {0: 40, 1: 45, 2: 50, 3: 55, 4: 59, 5: 64}
  token_sequence = []

  # Extract notes and convert to string/fret tokens
  annos = jam.search(namespace='note_midi')
  if len(annos) == 0:
    annos = jam.search(namespace='pitch_midi')
  ...
  for s, string_tran in enumerate(annos):
    for note in string_tran:
      ...
      fret = int(round(midi_note - str_midi_dict[s]))
      token_sequence.append(f'<S{s}><F{fret}>')

  # Extract chords and add <C> and </C> tokens
  chord_ann = jam.search(namespace='chord')
  if len(chord_ann) > 0:
    for chord in chord_ann[0].data:
      start_time = chord.time
      token_sequence.append('<C>')
      for s, string_tran in enumerate(annos):
        for note in string_tran:
          if abs(note.time - start_time) < 0.05:  # If the note is close to chord onset
            midi_note = note.value
            fret = int(round(midi_note - str_midi_dict[s]))
            token_sequence.append(f'<S{s}><F{fret}>')
      token_sequence.append('</C>')
...
jam = jams.load('data/guitarset-data/annotation/00_Rock2-142-D_comp.jams')

print(tokenize_jams(jam))
